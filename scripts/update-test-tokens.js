...
async function updateTestTokens() {
  ...
  try {
    ...
    const authOutput = execSync("NODE_TLS_REJECT_UNAUTHORIZED=0 timeout 60 bun run src/bin/cli.ts auth --config examples/oauth2-config.yml --provider gitlab --timeout 50", {
      encoding: "utf8",
      stdio: "pipe",
    });
