...
export class ConfigValidator {
  private validators = [new GitlabConfigValidator(), new DatabaseConfigValidator(), new OutputConfigValidator(), new LoggingConfigValidator()];
...
export class CallbackProcessor implements DataProcessor {
  ...
  async process<T>(data: T[], context: ProcessingContext): Promise<T[]> {
    try {
      ...
      const processedData = await callbackManager.processObjects(context.callbackContext, data);

      // Log filtering statistics
      if (processedData.length !== data.length) {
        context.logger.info(`${context.resourceType}: ${data.length} original → ${processedData.length} processed (${data.length - processedData.length} filtered)`);
      }
...
export class CrawlValidation {
  ...
  static validateResult(result: CrawlResult): boolean {
    return typeof result.success === "boolean" && typeof result.totalProcessingTime === "number" && result.summary !== undefined;
  }
}
```
...
export abstract class BaseCrawlStrategy implements CrawlStrategy {
  ...
  protected createResult(success: boolean, processingTime: number, summary: any, errors?: Error[]): CrawlResult {
    return {
      success,
...
export abstract class BaseStep {
  ...
  protected createStepResult(success: boolean, resourcesCrawled: number, processingTime: number, errors: string[] = []): StepResult {
    return {
      success,
      resourcesCrawled,
      processingTime,
