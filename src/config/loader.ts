...
import type { Config, CliArgs, EnvMapping } from './types.js';
...
export class ConfigLoader {
    ...
    for (const configPath of localConfigPaths) {
      if (existsSync(configPath)) {
        try {
          ...
          logger.warn(`Failed to load local config from ${configPath}:`, error);
        }
      }
    }
    ...
    for (const configPath of userConfigPaths) {
      if (existsSync(configPath)) {
        try {
          ...
          logger.debug(`Loaded user config from ${configPath}`);
          return;
        } catch (error) {
          logger.warn(`Failed to load user config from ${configPath}:`, error);
        }
      }
    }
  ...
  private loadEnvironmentVariables(): void {
    ...
    if (Object.keys(envConfig).length > 0) {
      ...
      logger.debug('Applied COPIMA_* environment variable configuration');
    }
  }
  /**
  ...
  private loadCliArguments(args: CliArgs): void {
    ...
    if (args.accessToken) {
      argsConfig.gitlab = { ...argsConfig.gitlab, ...this.config.gitlab, accessToken: args.accessToken };
    }
    if (args.refreshToken) {
      argsConfig.gitlab = { ...argsConfig.gitlab, ...this.config.gitlab, refreshToken: args.refreshToken };
    }
    if (args.timeout) {
      argsConfig.gitlab = { ...argsConfig.gitlab, ...this.config.gitlab, timeout: args.timeout };
    }
    if (args.maxConcurrency) {
      argsConfig.gitlab = { ...argsConfig.gitlab, ...this.config.gitlab, maxConcurrency: args.maxConcurrency };
    }
    if (args.rateLimit) {
      argsConfig.gitlab = { ...argsConfig.gitlab, ...this.config.gitlab, rateLimit: args.rateLimit };
    }

    // Database configuration
    if (args.databasePath) {
      argsConfig.database = { ...this.config.database, path: args.databasePath };
    }
    if (args.walMode !== undefined) {
      argsConfig.database = { ...argsConfig.database, ...this.config.database, walMode: args.walMode };
    }
    if (args.databaseTimeout) {
      argsConfig.database = { ...argsConfig.database, ...this.config.database, timeout: args.databaseTimeout };
    }
    ...
    if (args.fileNaming) {
      argsConfig.output = { ...argsConfig.output, ...this.config.output, fileNaming: args.fileNaming as 'lowercase' | 'kebab-case' | 'snake_case' };
    }
    if (args.prettyPrint !== undefined) {
      argsConfig.output = { ...argsConfig.output, ...this.config.output, prettyPrint: args.prettyPrint };
    }
    if (args.compression) {
      argsConfig.output = { ...argsConfig.output, ...this.config.output, compression: args.compression as 'none' | 'gzip' | 'brotli' };
    }

    // Logging configuration
    if (args.logLevel) {
      argsConfig.logging = { ...this.config.logging, level: args.logLevel as 'info' | 'error' | 'warn' | 'debug' };
    }
    if (args.logFormat) {
      argsConfig.logging = { ...argsConfig.logging, ...this.config.logging, format: args.logFormat as 'json' | 'simple' | 'combined' };
    }
    if (args.logFile) {
      argsConfig.logging = { ...argsConfig.logging, ...this.config.logging, file: args.logFile };
    }
    if (args.console !== undefined) {
      argsConfig.logging = { ...argsConfig.logging, ...this.config.logging, console: args.console };
    }
    if (args.colors !== undefined) {
      argsConfig.logging = { ...argsConfig.logging, ...this.config.logging, colors: args.colors };
    }

    // Progress configuration
    if (args.progressEnabled !== undefined) {
      argsConfig.progress = { ...this.config.progress, enabled: args.progressEnabled };
    }
    if (args.progressFile) {
      argsConfig.progress = { ...argsConfig.progress, ...this.config.progress, file: args.progressFile };
    }
    if (args.progressInterval) {
      argsConfig.progress = { ...argsConfig.progress, ...this.config.progress, interval: args.progressInterval };
    }
    if (args.progressDetailed !== undefined) {
      argsConfig.progress = { ...argsConfig.progress, ...this.config.progress, detailed: args.progressDetailed };
    }

    // Resume configuration
    if (args.resumeEnabled !== undefined) {
      argsConfig.resume = { ...this.config.resume, enabled: args.resumeEnabled };
    }
    if (args.resumeStateFile) {
      argsConfig.resume = { ...argsConfig.resume, ...this.config.resume, stateFile: args.resumeStateFile };
    }
    if (args.resumeAutoSaveInterval) {
      argsConfig.resume = { ...argsConfig.resume, ...this.config.resume, autoSaveInterval: args.resumeAutoSaveInterval };
    }

    if (Object.keys(argsConfig).length > 0) {
      this.mergeConfig(argsConfig);
      logger.debug('Applied CLI argument configuration');
    }
  }
